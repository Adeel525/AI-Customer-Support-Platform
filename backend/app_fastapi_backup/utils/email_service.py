...
class EmailService:
    ...
    async def send_verification_email(self, to: str, token: str) -> bool:
        ...
        return await self.send_email(to, "Verify your SupportAI account", html)

    async def send_password_reset_email(self, to: str, token: str) -> bool:
        url = f"{settings.FRONTEND_URL}/reset-password?token={token}"
        html = f"<p>Reset your password: <a href='{url}'>{url}</a></p>"
        return await self.send_email(to, "Reset your SupportAI password", html)

    async def send_invite_email(self, to: str, workspace_name: str, inviter: str) -> bool:
        url = f"{settings.FRONTEND_URL}/signup"
        html = f"<p>{inviter} invited you to join {workspace_name}. <a href='{url}'>Sign up</a></p>"
        return await self.send_email(to, f"Invitation to {workspace_name}", html)
